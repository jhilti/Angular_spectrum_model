...
from .dmso_mixture import dmso_water_properties
...
def _quadratic_peak_position(x: np.ndarray, amplitude: np.ndarray) -> float:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Focus sweep for 70-100 percent DMSO/water at 22 degC."
    )
...
def run(args: argparse.Namespace) -> list[dict[str, float | str | bool]]:
    ...
    aperture = FocusedCircularAperture(13.0e-3, 25.0e-3)
    plate = ElasticPlate(
    ...
    for concentration_percent in concentrations:
        ...
        focus_in_mixture_m = _quadratic_peak_position(
        ...
        rows.append(
            {
                ...
                "focus_inside_mixture": focus_in_mixture_m <= dmso_height_m,
    ...
    axes[0].axhline(
        ...
        color="0.45",
        linestyle="--",
        label="Oberkante der 4,22-mm-Schicht",
    )
    axes[0].set(
        ylabel="Fokus ab PP-Rückseite [mm]",
        title=f"Fokuslage bei {args.dmso_height_mm:.2f} mm DMSO/Wasser",
    )
    ...
    axes[1].set(
        ...
        ylabel="Fokus unter Oberkante [mm]",
    )
    axes[1].grid(alpha=0.25)
    speed_axis = axes[1].twinx()
    speed_axis.plot(concentration, speed, color="tab:orange", alpha=0.65)
    speed_axis.set_ylabel("Schallgeschwindigkeit [m/s]", color="tab:orange")
...
def main() -> None:
    ...
    rows = run(args)
    print("DMSO [%]  c [m/s]  Fokus ab PP [mm]  unter Oberkante [mm]")
    for row in rows:
